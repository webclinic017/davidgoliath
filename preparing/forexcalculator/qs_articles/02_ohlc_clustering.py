...
def get_open_normalised_prices(symbol, start, end):
    ...
    """
    # # get online data
    # df = iv.indices.get_index_historical_data(symbol, 'united states',
...
if __name__ == "__main__":
    ...
    symbol = "S&P 500"
    start = convert_date(datetime.datetime(2000, 1, 1))
    end = convert_date(datetime.datetime(2021, 7, 8))
    '''
    '''

    # ---------- BONUS ----------
    # sp500 = web.DataReader(symbol, "yahoo", start, end)
    # sp500 = iv.indices.get_index_historical_data(symbol, 'united states',
    #                  from_date=start, to_date=end)

    # ---------- Speedup by read data ----------
    # read data
    sp500 = pd.read_csv(f'data/{symbol}.csv')
    sp500['Date'] = pd.to_datetime(sp500['Date'])
    sp500.set_index('Date', inplace=True)
    # Plot last year of price "candles"
    plot_candlesticks(sp500, convert_date(datetime.datetime(2020, 7, 8)))
    # ---------- END BONUS ----------
