...
def getEconomicCalendar(startlink):
    ...
    fields = ["date", "time", "currency", "impact",
    ...
    for tr in trs:
        ...
        try:
            for field in fields:
                data = tr.select(
                    "td.calendar__cell.calendar__{}.{}".format(field, field))[0]
                # print(data)
            ...
            dt = datetime.datetime.strptime(",".join([curr_year, curr_date, curr_time]),
                                            "%Y,%a%b %d,%I:%M%p")
            print(",".join([str(dt), currency, impact,
                  event, actual, forecast, previous]))
...
if __name__ == "__main__":
    ...
    """
    # setLogger()
    # getEconomicCalendar("calendar.php?week=jul18.2021",
    #                     "calendar.php?week=jul23.2021")
    getEconomicCalendar("calendar.php?week=jul18.2021")
